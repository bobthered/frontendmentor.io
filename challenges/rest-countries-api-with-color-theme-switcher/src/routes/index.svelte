...
<svelte:head>
  <title>
    REST Countries API with Color Theme Switcher - Frontendmentor.io
  </title>
</svelte:head>

<Container>
  <div
    class="flex flex-col space-y-[38px] lg:flex-row lg:space-y-0 lg:justify-between">
    <div class="relative text-light-input dark:text-dark-input lg:w-[480px]">
      <Input bind:value={search} />
      <Search
        class="w-[20px] h-[20px] absolute top-1/2 transform translate-y-[-50%] left-[31px]" />
    </div>
    <div class="w-[200px]">
      <Dropdown
        bind:current={region}
      ...
      <Card>
        <a href="{base}/{name.toLowerCase().replace(/\s/g, '-')}">
          ...
          <div class="flex flex-col p-[25px] text-[14px] leading-[24px]">
            ...
            <div class="mt-[16px]">
              <span class="font-semibold">Population:
              </span>{population
                .toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
            </div>
            <div class="">
              <span class="font-semibold">Region: </span>{countryRegion}
            </div>
            <div class="">
              <span class="font-semibold">Captal: </span>{capital}
            </div>
          </div>
        </a>
      </Card>
